...
func TestFrameProp_MarshalXML(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: FrameProp{
				Width:      new(int64(500)),
				Height:     new(int64((300))),
				DropCap:    new(stypes.DropCapMargin),
				Lines:      new(3),
				VSpace:     new(int64((50))),
				HSpace:     new(int64((20))),
				Wrap:       new(stypes.WrapAround),
				HAnchor:    new(stypes.AnchorMargin),
				VAnchor:    new(stypes.AnchorPage),
				AbsHPos:    new(100),
				AbsVPos:    new(200),
				XAlign:     new(stypes.XAlignLeft),
				YAlign:     new(stypes.YAlignCenter),
				HRule:      new(stypes.HeightRuleExact),
				AnchorLock: new(stypes.OnOffTrue),
			},
